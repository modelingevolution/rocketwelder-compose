...
# the Blazor Performance Monitor layout in appsettings.linux.json
#
...
# Configuration
# MonitorSettings goes into appsettings.linux.json (the OS-specific config file the
# app loads on Linux). NOT appsettings.runtime.json — the app rewrites that file on
# startup and up-1.0.0.sh resets it to {}, so MonitorSettings would not survive.
APPSETTINGS_PATH="/var/docker/data/app/appsettings.linux.json"
COMPOSE_OVERRIDE="/var/docker/configuration/rocket-welder/docker-compose.override.yml"
...
Blazor Performance Monitor layout in appsettings.linux.json.
...
update_appsettings() {
    log_info "=== Updating $(basename "$APPSETTINGS_PATH") ==="
...
# Ensure the gitignored docker-compose.override.yml bind-mounts appsettings.linux.json
# into the app container. AutoUpdater auto-includes every docker-compose*.yml file,
# and .gitignore excludes docker-compose.override.yml, so it is device-local and
# survives repository updates.
ensure_compose_override() {
    log_info "=== Ensuring docker-compose.override.yml mount ==="

    local compose_dir
    compose_dir="$(dirname "$COMPOSE_OVERRIDE")"
    if [ ! -d "$compose_dir" ]; then
        log_warn "Compose directory not found: $compose_dir - skipping override file"
        return 0
    fi

    if [ -f "$COMPOSE_OVERRIDE" ] && grep -q "appsettings.linux.json" "$COMPOSE_OVERRIDE"; then
        log_info "✓ docker-compose.override.yml already mounts appsettings.linux.json"
        return 0
    fi

    if [ -f "$COMPOSE_OVERRIDE" ]; then
        log_warn "$COMPOSE_OVERRIDE exists without an appsettings.linux.json mount"
        log_warn "Add this volume to the 'modelingevolution.rocketwelder' service:"
        log_warn "  - \"$APPSETTINGS_PATH:/app/appsettings.linux.json\""
        return 0
    fi

    cat > "$COMPOSE_OVERRIDE" <<EOF
# Device-local override (gitignored). Generated by configure-perfmon.sh.
# AutoUpdater auto-includes every docker-compose*.yml file in this directory.
name: rocket-welder
services:
  modelingevolution.rocketwelder:
    volumes:
      - "$APPSETTINGS_PATH:/app/appsettings.linux.json"
EOF
    log_info "✓ Created $COMPOSE_OVERRIDE"
}
...
main() {
    ...
    # Ensure the app container mounts appsettings.linux.json
    ensure_compose_override
    ...
    log_info "  cd /var/docker/configuration/rocket-welder && sudo docker compose up -d"
}
